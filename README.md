...
<p align="center">
  <img src="./lua-config.svg" alt="lua-config logo" width="128" />
</p>

<h1 align="center">@sebastienrousseau/lua-config</h1>
...
<p align="center">
  <a href="https://github.com/sebastienrousseau/lua-config/actions"><img src="https://img.shields.io/github/actions/workflow/status/sebastienrousseau/lua-config/ci.yml?branch=main&style=for-the-badge&logo=github" alt="Build Status" /></a>
  <a href="https://www.npmjs.com/package/@sebastienrousseau/lua-config"><img src="https://img.shields.io/npm/v/%40sebastienrousseau%2Flua-config.svg?style=for-the-badge&color=fc8d62&logo=npm" alt="npm package" /></a>
  <a href="https://sebastienrousseau.com/lua-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
  <a href="https://scorecard.dev/viewer/?uri=github.com/sebastienrousseau/lua-config"><img src="https://img.shields.io/ossf-scorecard/github.com/sebastienrousseau/lua-config?style=for-the-badge&label=OpenSSF%20Scorecard&logo=openssf" alt="OpenSSF Scorecard" /></a>
...
- [Hardened LuaCheck & StyLua Standards](#hardened-luacheck-stylua-standards) — before and after rule comparison
...
Install `@sebastienrousseau/lua-config` as a development dependency:
...
# pnpm
pnpm add -D @sebastienrousseau/lua-config

# yarn
yarn add -D @sebastienrousseau/lua-config

# bun
bun add -d @sebastienrousseau/lua-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/lua-config.git
cd lua-config
make            # check + test
```
...
- **Target environment**: Optimized for Lua 5.4 / StyLua.
...
```bash
cp node_modules/@sebastienrousseau/lua-config/.luacheckrc ./.luacheckrc
cp node_modules/@sebastienrousseau/lua-config/stylua.toml ./stylua.toml
```

### Or point the tool at the installed file

```bash
luacheck --config node_modules/@sebastienrousseau/lua-config/.luacheckrc src
stylua --config-path node_modules/@sebastienrousseau/lua-config/stylua.toml src
```
...
`@sebastienrousseau/lua-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/lua-config` solves this with three deliberate engineering choices:
...
## Hardened LuaCheck & StyLua Standards

Enforces strict variable scoping, eliminates accidental global leakage, and formats Lua files with deterministic 2-space indentation.
...
### After (@sebastienrousseau/lua-config Enforced)
...
When developing for legacy Lua 5.0 systems with incompatible syntax or scoping semantics.
