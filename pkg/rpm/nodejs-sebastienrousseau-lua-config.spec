...
%global npm_bare      lua-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-lua-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Lua configuration providing standardized LuaCheck and StyLua rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/lua-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/lua-config is a shareable configuration preset with no runtime
...
install -pm 0644 .luacheckrc %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 stylua.toml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
